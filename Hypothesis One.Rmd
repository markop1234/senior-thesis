...
survey$EP_1 <- as.numeric(survey$EP_1)
survey$EP_2 <- as.numeric(survey$EP_2)
survey$EP_3 <- as.numeric(survey$EP_3)
survey$EP_4 <- as.numeric(survey$EP_4)
survey$EP_5 <- as.numeric(survey$EP_5)
survey$SP_1 <- as.numeric(survey$SP_1)
survey$SP_2 <- as.numeric(survey$SP_2)
survey$SP_3 <- as.numeric(survey$SP_3)
survey$SP_4 <- as.numeric(survey$SP_4)
survey$SP_5 <- as.numeric(survey$SP_5)
survey$GP_1 <- as.numeric(survey$GP_1)
survey$GP_2 <- as.numeric(survey$GP_2)
survey$GP_3 <- as.numeric(survey$GP_3)
survey$GP_4 <- as.numeric(survey$GP_4)
survey$GP_5 <- as.numeric(survey$GP_5)
...
4: Create indices
```{r}
survey$EP_Index <- rowMeans(survey[, c("EP_1", "EP_2", "EP_3", "EP_4", 
                                     "EP_5")])
survey$EP_z_Index <- rowMeans(survey[, c("EP_1_z", "EP_2_z", "EP_3_z", "EP_4_z", 
                                     "EP_5_z")])
survey$SP_Index <- rowMeans(survey[ , c("SP_1", "SP_2", "SP_3", "SP_4",
                                      "SP_5")])
survey$SP_z_Index <- rowMeans(survey[ , c("SP_1_z", "SP_2_z", "SP_3_z", "SP_4_z",
                                      "SP_5_z")])
survey$GP_Index <- rowMeans(survey[ , c("GP_1", "GP_2", "GP_3", "GP_4",
                                      "GP_5")])
survey$GP_z_Index <- rowMeans(survey[ , c("GP_1_z", "GP_2_z", "GP_3_z", "GP_4_z",
                                      "GP_5_z")])
survey$ESG_Index <- rowMeans(survey[ , c("SP_1", "SP_2", "SP_3", "SP_4",
                                      "SP_5", "EP_1", "EP_2", "EP_3", "EP_4", 
                                     "EP_5", "GP_1", "GP_2", "GP_3", "GP_4",
                                      "GP_5")])
survey$ESG_z_Index <- rowMeans(survey[ , c("SP_1_z", "SP_2_z", "SP_3_z", "SP_4_z",
                                      "SP_5_z", "EP_1_z", "EP_2_z", "EP_3_z", "EP_4_z", 
                                     "EP_5_z", "GP_1_z", "GP_2_z", "GP_3_z", "GP_4_z",
                                      "GP_5_z")])
survey$ESGF <- rowMeans(survey[ , c("EPF", "SPF", "GPF")])
survey$ESGF_z <- rowMeans(survey[ , c("EPF_z", "SPF_z", "GPF_z")])
```
...
TABLE2 <- survey %>%
  select(EP_Index, SP_Index, GP_Index, ESG_Index, Size) %>%
  group_by(Size) %>%
  summarise(`Environmental Pillar`=mean(EP_Index), `Social Pillar`=mean(SP_Index), `Governmental Pillar`=mean(GP_Index), `ESG Overall`=mean(ESG_Index)) %>%
  slice(match(size_order, Size))

formattable(TABLE2, list(`Size` = formatter("span", 
  style = ~ style(color = "grey",font.weight = "bold")), 
  `EP`= color_tile(customGreen, customGreen0),
  `SP`= color_tile(customGreen, customGreen0),
  `GP`= color_tile(customGreen, customGreen0),
  `ESG`= color_tile(customGreen, customGreen0)
),
table.attr = 'style="font-family: Times New Roman";\"')
...
TABLE3 <- survey %>%
  ...
  group_by(Employees) %>%
  summarise(EP=mean(EP_Index), SP=mean(SP_Index), GP=mean(GP_Index), ESG=mean(ESG_Index)) %>%
  slice(match(employee_order, Employees))

formattable(TABLE3, list(`Employees` = formatter("span", 
  style = ~ style(color = "grey",font.weight = "bold")), 
  `EP`= color_tile(customGreen, customGreen0),
  `SP`= color_tile(customGreen, customGreen0),
  `GP`= color_tile(customGreen, customGreen0),
...
7: Scatter plots
```{r}
plot(survey.num$Employees, survey.num$ESG_Index)
plot(survey.num$Size, survey.num$ESG_Index)
```

8: Correlations
```{r}
cor(survey$ESG_Index, survey.num$Employees)
cor(survey$ESG_Index, survey.num$Size)

cov(survey.num$Size, survey.num$Employees, use = "complete.obs")
vif(EP)

## Cronbach alpha
alpha <- tibble(survey$EPF, survey$SPF, survey$GPF)
cronbach.alpha(alpha, na.rm=TRUE)
```

9: Regressions
```{r}
# Model with 'size'
EP <- lm(EP_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
SP <- lm(SP_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
GP <- lm(GP_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
ESG <- lm(ESG_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
tab_model(ESG, EP, SP, GP, 
          title = "TABLE 2: Regression of perceived ESG indices by self-identified firm size",
          file = "table_2.doc")
...
## REFERENCE RACE: American Indian/Native American or Alaska Native
## REFERENCE GENDER: Non-binary / third gender

# Model with 'employees'
EP.e <- lm(EP_Index ~ Employees + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
SP.e <- lm(SP_Index ~ Employees + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
GP.e <- lm(GP_Index ~ Employees + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
ESG.e <- lm(ESG_Index ~ Employees + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey)
tab_model(ESG.e, EP.e, SP.e, GP.e,
          title = "TABLE 3: Regression of perceived ESG indices by employee count",
          file = "table_3.doc")

# Treat as numeric
EP.n <- lm(EP_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey.num)
SP.n <- lm(SP_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey.num)
GP.n <- lm(GP_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey.num)
ESG.n <- lm(ESG_Index ~ Size + Role + Gender + Political_Identity
         + Years + Education + Income + Age + Race + Ethnicity, data = survey.num)
tab_model(ESG.n, EP.n, SP.n, GP.n)
```